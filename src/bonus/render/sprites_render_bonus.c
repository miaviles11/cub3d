...
/*   Updated: 2025/09/25 16:59:03 by miaviles         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sprite_stripe(t_cub *c, t_sprite *sp, int screen_x, int sprite_w,
		int start_y, int end_y, int sprite_h)
{
	int		start_x;
	int		end_x;
	int		x;
	...
	double ty = sp->distance;
	jump_offset = (int)(c->player.z_offset * JUMP_VISUAL_MULTIPLIER);
	v_move_screen = (int)(SPRITE_VMOVE / ty);
	for (x = start_x; x <= end_x; ++x)
	{
		if (x < 0 || x >= WIN_W)
			continue ;
		if (!(ty > 0.0 && ty < c->z_buffer[x]))
			continue ;
		tex_x = (int)((x - start_x) * TEX_SIZE / (double)sprite_w);
		if (tex_x < 0)
			tex_x = 0;
		if (tex_x >= TEX_SIZE)
			tex_x = TEX_SIZE - 1;
		v_step = (double)TEX_SIZE / (double)sprite_h;
		v_pos = (start_y - WIN_H / 2.0 - jump_offset - v_move_screen + sprite_h
				/ 2.0) * v_step;
		for (int y = start_y; y <= end_y; ++y)
		{
			...
			if (tex_y >= 0 && tex_y < TEX_SIZE)
			{
				...
				color = tex_sample(c, tex_id, tex_x, tex_y);
		...
		double transform_y)
{
	...
	if (draw_end_y >= WIN_H)
		draw_end_y = WIN_H - 1;
	draw_sprite_stripe(cub, sprite, sprite_screen_x, sprite_w, draw_start_y,
		draw_end_y, sprite_h);
}
